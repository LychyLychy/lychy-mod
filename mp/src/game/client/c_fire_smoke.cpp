...
ConVar lychy_fire_old("lychy_fire_old", "0");
ConVar lychy_fire_embers("lychy_fire_embers", "0");
ConVar lychy_fire_overlay("lychy_fire_overlay", "0");
ConVar lychy_fire_particles("lychy_fire_particles", "0");
...
{
#if PARTICLE_FIRE
	if (lychy_fire_particles.GetBool())
	{
...
{
	if (m_pEmberEmitter.IsValid() && lychy_fire_embers.GetBool())
	{
...
void C_FireSmoke::SpawnSmoke(void)
{
	...
	if (m_pEmberEmitter.IsValid() == false && lychy_fire_embers.GetBool())
		return;
