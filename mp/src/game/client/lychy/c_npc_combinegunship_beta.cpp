...
class C_GunshipFXBeta : public C_EnvelopeFX
{
public:
...
// Purpose: 
//-----------------------------------------------------------------------------
C_GunshipFXBeta::C_GunshipFXBeta(void)
{
	m_pOwner = NULL;
}
...
class CGunshipFXEnvelopeBeta
{
public:
	CGunshipFXEnvelopeBeta();
...
CGunshipFXEnvelopeBeta::CGunshipFXEnvelopeBeta()
{
	// Glow flare
...
CGunshipFXEnvelopeBeta g_GunshipCannonEnvelope;
...
static void DrawSpriteTangentSpace(const Vector& vecOrigin, float flWidth, float flHeight, color32 color)
{
	...
	Vector point;
	CMatRenderContextPtr pMat(materials);
	IMesh* pMesh = pMat->GetDynamicMesh();
...
static void Gunship_DrawSprite(const Vector& vecOrigin, float size, const color32& color, bool glow)
{
	if (glow)
	{
		if (GlowSightDistance(vecOrigin,false) <= 0)
			return;
	}
...
// Input  : int - 
//-----------------------------------------------------------------------------
int	C_GunshipFXBeta::DrawModel(int)
{
	...
	if (hasParam[GUNSHIPFX_FLARE_COLOR] && hasParam[GUNSHIPFX_FLARE_SIZE])
	{
		...
		pContext->Bind(pMat, (IClientRenderable*)this);
...
//			&targetPos - 
//-----------------------------------------------------------------------------
void C_GunshipFXBeta::Update(C_BaseEntity* pOwner, const Vector& targetPos)
{
	BaseClass::Update();
...
class C_CombineGunshipBeta : public C_BaseHelicopter
{
	DECLARE_CLASS(C_CombineGunshipBeta, C_BaseHelicopter);
public:
	DECLARE_CLIENTCLASS();

	C_CombineGunshipBeta(void) {}

	virtual ~C_CombineGunshipBeta(void)
	{
		m_cannonFX.EffectShutdown();
	}

	C_GunshipFXBeta	m_cannonFX;
	Vector		m_vecHitPos;
...
private:
	C_CombineGunshipBeta(const C_CombineGunshipBeta&) {}
};

IMPLEMENT_CLIENTCLASS_DT(C_CombineGunshipBeta, DT_CombineGunshipBeta, CNPC_CombineGunshipBeta)
...
//-----------------------------------------------------------------------------
static void ImpactGunshipCallback(const CEffectData& data)
{
	trace_t tr;
	Vector vecOrigin, vecStart, vecShotDir;
	int iMaterial, iDamageType, iHitbox/*, iEntIndex*/;
	short nSurfaceProp;
	C_BaseEntity* pEntity = ParseImpactData(data, &vecOrigin, &vecStart, &vecShotDir,nSurfaceProp, iMaterial, iDamageType, iHitbox/*, &iEntIndex*/);
