...
BEGIN_DATADESC( CNPC_RollerBuddy )
	...
	DEFINE_FIELD(  m_vecCommandLocation,	FIELD_VECTOR ),
	DEFINE_FIELD(  m_hCommandEntity,		FIELD_EHANDLE ),
	DEFINE_FIELD(  m_pMaster,				FIELD_CLASSPTR ),
...
void CNPC_RollerBuddy::Spawn( void )
{
	...
	//m_pRollSound = controller.SoundCreate( filter, entindex(), CHAN_AUTO, ROLLERBUDDY_SOUND_MOVE, ATTN_NORM );
}
	...
	case TASK_ROLLERBUDDY_ACTIVATE_COMMAND_ENTITY:
		if( gpGlobals->curtime >= m_flWaitFinished )
		{
			...
			else
			{
				...
				CPASAttenuationFilter filter( this );
				EmitSound_t emit;
				emit.m_pSoundName = pCodeSounds[m_iAccessCode[m_iCodeProgress]];
				emit.m_flVolume = 1.0;
				emit.m_nChannel = CHAN_BODY;

				EmitSound( filter, entindex(), emit );
				m_iCodeProgress++;
			}
		}
		break;
...
void CNPC_RollerBuddy::Precache( void )
{
	...
	PrecacheScriptSound("NPC_RollerBuddy.Confirm");
	PrecacheScriptSound("NPC_RollerBuddy.RemoteNo");
	PrecacheScriptSound("NPC_RollerBuddy.RemoteYes");
}
