...
ConVar lychy_rollerbull_debug("lychy_rollerbull_debug", "0");
...
BEGIN_DATADESC( CNPC_RollerBull )
	...
	DEFINE_FIELD( m_fHackVisibilityFlag, FIELD_BOOLEAN ),
	DEFINE_FIELD( m_fEnableEnemy, FIELD_BOOLEAN ),
	DEFINE_FIELD( m_iSoundEventFlags, FIELD_INTEGER ),
	DEFINE_SOUNDPATCH(  m_pRollSoundFast ),
...
{
	engine->PrecacheModel( "models/roller_beta.mdl" );
	engine->PrecacheModel( "models/roller_beta_spikes.mdl" );
	...
	PrecacheScriptSound("NPC_RollerBull.Measure");
	PrecacheScriptSound("NPC_RollerBull.Tossed");
	PrecacheScriptSound("NPC_RollerBull.Taunt");
	PrecacheScriptSound("NPC_RollerBull.OpenSpikes");
	PrecacheScriptSound("NPC_RollerBull.CloseSpikes");
	PrecacheScriptSound("NPC_RollerBull.Cut");
	PrecacheScriptSound("NPC_RollerBull.Summon");
	PrecacheScriptSound("NPC_RollerBull.Idle");
	PrecacheScriptSound("NPC_RollerBull.Bark");
	PrecacheScriptSound("NPC_RollerBull.Measure");
	...
	case TASK_ROLLERBULL_CHARGE_ENEMY:
		{
			...
			if(lychy_rollerbull_debug.GetBool())
...
{
	SetModel( "models/roller_beta_spikes.mdl" );
...
{
	SetModel( "models/roller_beta.mdl" );
...
void CNPC_RollerBull::PowerOn( void )
{
	...
	CPASAttenuationFilter filter(this);
	if (!m_pRollSound)
		m_pRollSound = controller.SoundCreate(filter, entindex(), CHAN_AUTO,ROLLERBULL_SOUND_MOVE, ATTN_NORM);
...
(
	...
	"	Tasks"
	"		TASK_ROLLERBULL_WAIT				2"
	"		TASK_ROLLERBULL_SOUND			1"
	"		TASK_ROLLERBULL_WAIT				5"
...
(
	...
	"	Tasks"
	"		TASK_ROLLER_OFF				0"
	"		TASK_ROLLERBULL_WAIT				1"
	"		TASK_ROLLERBULL_INSPECT			0"
	"		TASK_ROLLERBULL_WAIT				3"
