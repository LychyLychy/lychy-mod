...
ConVar lychy_rollerdozer_debug("lychy_rollerdozer_debug", "0");
ConVar lychy_rollerdozer_pickup("lychy_rollerdozer_pickup", "0");
//=========================================================
// Custom schedules
//=========================================================
enum
...
CBaseEntity *CNPC_RollerDozer::FindDebris( void )
{
	if( !GetHintNode() )
	{
		...
		CAI_Hint* pHintNode = CAI_HintManager::FindHint( this, HINT_ROLLER_CLEANUP_POINT, 0, 1024 ); 
		...
		if( pHintNode)
		{
			...
			for( i = 0 ; i < count ; i++ )
			{
				...
				if( pPhysObj->GetMass() <= 400 )
				{
					...
					if( lychy_rollerdozer_pickup.GetBool())
					{
			...
			{
				SetCondition( COND_ROLLERDOZER_FOUND_DEBRIS );
				if(lychy_rollerdozer_debug.GetBool())
