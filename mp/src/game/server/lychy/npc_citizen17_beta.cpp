...
extern ConVar	sk_citizen_heal_player			;//( "sk_citizen_heal_player",			"25");
extern ConVar	sk_citizen_heal_player_delay	;//( "sk_citizen_heal_player_delay",	"30");
extern ConVar	sk_citizen_heal_player_min_pct	;//( "sk_citizen_heal_player_min_pct",	"0.60");
extern ConVar	sk_citizen_heal_ally			;//( "sk_citizen_heal_ally",			"30");
extern ConVar	sk_citizen_heal_ally_delay		;//( "sk_citizen_heal_ally_delay",		"20");
extern ConVar	sk_citizen_heal_ally_min_pct	;//( "sk_citizen_heal_ally_min_pct",	"0.90");
const float HEAL_RANGE = 15*12;

//=========================================================
// Citizen activities
//=========================================================
int ACT_CITBETA_HANDSUP;
int ACT_CITBETA_HANDSDOWN;
int ACT_CITBETA_HANDSUPIDLE;
int	ACT_CITBETA_BLINDED;
int ACT_CITBETA_SHOWARMBAND;
int ACT_CITBETA_COWERFLINCH;
int ACT_CITBETA_COWERIDLE;
int ACT_CITBETA_COWERSTAND;
int ACT_CITBETA_COWER;
int ACT_CITBETA_IDLEWALL;
int ACT_CITBETA_LEANONWALL;
int ACT_CITBETA_LEANOFFWALL;
int ACT_CITBETA_MANHACKFLINCH;
int ACT_CITBETA_HEAL;

//=========================================================
// Static Data
//=========================================================
extern ConVar	sk_citizen_health;// ("sk_citizen_health", "0");


LINK_ENTITY_TO_CLASS( npc_citizen_beta, CNPC_CitizenBeta );

//---------------------------------------------------------
// Save/Restore
//---------------------------------------------------------

BEGIN_DATADESC( CNPC_CitizenBeta )

	DEFINE_FIELD(m_nInspectActivity, FIELD_INTEGER ),
	...
	DEFINE_FIELD(m_hCurLookTarget, FIELD_EHANDLE ),
//	DEFINE_FIELD(m_nextHead, FIELD_INTEGER ),
//	DEFINE_FIELD(m_AssaultBehavior, FIELD_EMBEDDED ),
//	DEFINE_FIELD(m_FollowBehavior, FIELD_EMBEDDED ),
//	DEFINE_FIELD(m_StandoffBehavior, FIELD_EMBEDDED ),
...
FACTORYCMD(lychy_citizen_beta, "npc_citizen", "npc_citizen_beta")
//-----------------------------------------------------------------------------
// Purpose: 
//
//
//-----------------------------------------------------------------------------
void CNPC_CitizenBeta::Precache( void )
{
	if( !GetModelName() )
	{
...
// Purpose: 
//-----------------------------------------------------------------------------
string_t CNPC_CitizenBeta::GetModelName( void ) const
{
...
//-----------------------------------------------------------------------------
void CNPC_CitizenBeta::SelectCitizenModel()
{
...
//
//-----------------------------------------------------------------------------
int CNPC_CitizenBeta::m_nextHead = 0;
void CNPC_CitizenBeta::Spawn( void )
{
	SelectCitizenModel();
	...
	m_szAltClassName = AllocPooledString(GetClassname());
	SetClassname("npc_citizen_beta");
...
void CNPC_CitizenBeta::PostNPCInit( void )
{
...
bool CNPC_CitizenBeta::CreateBehaviors()
{
...
//-----------------------------------------------------------------------------
Class_T	CNPC_CitizenBeta::Classify( void )
{
...
//---------------------------------------------------------
//---------------------------------------------------------
bool CNPC_CitizenBeta::FCanCheckAttacks( void )
{
...
//---------------------------------------------------------
//---------------------------------------------------------
int CNPC_CitizenBeta::GetSoundInterests ( void )
{
	return	SOUND_WORLD			|
			SOUND_COMBAT		|
			SOUND_PLAYER		|
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_CitizenBeta::LocateEnemySound()
{
#if 1
	if ( !GetEnemy() )
		return;
...
// Output  :
//------------------------------------------------------------------------------
void CNPC_CitizenBeta::DeathSound (const CTakeDamageInfo& info)
{
...
// Output  :
//------------------------------------------------------------------------------
void CNPC_CitizenBeta::PainSound (const CTakeDamageInfo& info)
{
	EmitSound( "npc_citizen.die" );
}
...
// Output  : 
//------------------------------------------------------------------------------
void CNPC_CitizenBeta::FearSound ( void )
{
}


bool CNPC_CitizenBeta::CanHeal( void )
{ 
	if ( !( m_spawnflags & SF_CITIZEN_MEDIC ) )
		return false;
...
bool CNPC_CitizenBeta::ShouldHealTarget( CBaseEntity *pTarget, bool bActiveUse )
{
	if ( pTarget )
	{
		if ( pTarget->IsPlayer() )
...
void CNPC_CitizenBeta::Heal( void )
{
	if ( !CanHeal() )
		  return;
...
// Output  :
//------------------------------------------------------------------------------
void CNPC_CitizenBeta::HandleAnimEvent( animevent_t *pEvent )
{
	switch( pEvent->event )
	{
...
//---------------------------------------------------------
//---------------------------------------------------------
int CNPC_CitizenBeta::SelectHealSchedule()
{
	if ( CanHeal() )
	{
...
//---------------------------------------------------------
//---------------------------------------------------------
int CNPC_CitizenBeta::SelectSchedule ( void )
{
...
//---------------------------------------------------------
//---------------------------------------------------------
int CNPC_CitizenBeta::TranslateSchedule( int scheduleType ) 
{
	switch( scheduleType )
	{
...
//---------------------------------------------------------
//---------------------------------------------------------
void CNPC_CitizenBeta::GatherConditions( void )
{
...
//-----------------------------------------------------------------------------
bool CNPC_CitizenBeta::ShouldStandoff( void )
{
...
//---------------------------------------------------------
//---------------------------------------------------------
void CNPC_CitizenBeta::PrescheduleThink( void )
{
...
// Output :
//-----------------------------------------------------------------------------
Activity CNPC_CitizenBeta::NPC_TranslateActivity( Activity activity )
{
...
// Input  : sHintType - 
//-----------------------------------------------------------------------------
Activity CNPC_CitizenBeta::GetHintActivity( short sHintType )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_CitizenBeta::SetLookTarget(CBaseEntity *pLookTarget)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
CBaseEntity *CNPC_CitizenBeta::EyeLookTarget()
{
...
bool CNPC_CitizenBeta::gm_bFindingTurretCover;

bool CNPC_CitizenBeta::FindCoverPos( CBaseEntity *pEntity, Vector *pResult)
{
...
bool CNPC_CitizenBeta::IsCoverPosition( const Vector &vecThreat, const Vector &vecPosition )
{
...
bool CNPC_CitizenBeta::ValidateNavGoal()
{
	bool result;
...
//---------------------------------------------------------
//---------------------------------------------------------
void CNPC_CitizenBeta::StartTask( const Task_t *pTask )
{
	switch( pTask->iTask )
	{
	...
	case TASK_CIT_HEAL:
		Speak( "TLK_HEAL" );
		SetIdealActivity( (Activity)ACT_CITBETA_HEAL );
		break;
...
//---------------------------------------------------------
//---------------------------------------------------------
void CNPC_CitizenBeta::RunTask( const Task_t *pTask )
{
	switch( pTask->iTask )
	{
		case TASK_SOUND_WAKE:
...
//---------------------------------------------------------
//---------------------------------------------------------
bool CNPC_CitizenBeta :: FValidateHintType ( CAI_Hint *pHint )
{
...
//---------------------------------------------------------
//---------------------------------------------------------
int CNPC_CitizenBeta::OnTakeDamage_Alive( const CTakeDamageInfo &info )
{
	if( info.GetAttacker() )
	{
...
//-----------------------------------------------------------------------------
bool CNPC_CitizenBeta::PassesDamageFilter(const CTakeDamageInfo& info)
{
...
//-----------------------------------------------------------------------------
void CNPC_CitizenBeta::BuildScheduleTestBits( void )
{
...
//-----------------------------------------------------------------------------
int CNPC_CitizenBeta::DrawDebugTextOverlays(void) 
{
...
//-----------------------------------------------------------------------------
bool CNPC_CitizenBeta::HandleInteraction(int interactionType, void *data, CBaseCombatCharacter* sourceEnt)
{
...
//---------------------------------------------------------
//---------------------------------------------------------
void CNPC_CitizenBeta::Touch( CBaseEntity *pOther )
{
...
//---------------------------------------------------------
//---------------------------------------------------------
bool CNPC_CitizenBeta::FInAimCone( const Vector &vecSpot )
{
...
//-----------------------------------------------------------------------------
void CNPC_CitizenBeta::MoveOrder( const Vector &vecDest )
{
...
//-----------------------------------------------------------------------------
bool CNPC_CitizenBeta::TargetOrder( CBaseEntity *pTarget )
{
...
void CNPC_CitizenBeta::Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
...
AI_BEGIN_CUSTOM_NPC( npc_citizen_beta, CNPC_CitizenBeta )
	...
	DECLARE_ACTIVITY(	ACT_CITBETA_HANDSUP)
	DECLARE_ACTIVITY(	ACT_CITBETA_HANDSDOWN)
	DECLARE_ACTIVITY(	ACT_CITBETA_HANDSUPIDLE)
	DECLARE_ACTIVITY(	ACT_CITBETA_BLINDED)
	DECLARE_ACTIVITY(	ACT_CITBETA_SHOWARMBAND)
	DECLARE_ACTIVITY(	ACT_CITBETA_COWERFLINCH)
	DECLARE_ACTIVITY(	ACT_CITBETA_COWERIDLE)
	DECLARE_ACTIVITY(	ACT_CITBETA_COWERSTAND)
	DECLARE_ACTIVITY(	ACT_CITBETA_COWER)
	DECLARE_ACTIVITY(	ACT_CITBETA_IDLEWALL)
	DECLARE_ACTIVITY(	ACT_CITBETA_LEANONWALL)
	DECLARE_ACTIVITY(	ACT_CITBETA_LEANOFFWALL)
	DECLARE_ACTIVITY(	ACT_CITBETA_MANHACKFLINCH)
	DECLARE_ACTIVITY(	ACT_CITBETA_HEAL)
	...
	(
		...
		"	Tasks"
		"		TASK_STOP_MOVING			0"
		"		TASK_SET_ACTIVITY			ACTIVITY:ACT_CITBETA_COWERIDLE"
		"		TASK_WAIT					5"
		"		TASK_SET_ACTIVITY			ACTIVITY:ACT_CITBETA_HANDSUPIDLE"
		"		TASK_WAIT					5"
		""
		"	Interrupts"
	)
	...
	(
		...
		"		TASK_SET_ACTIVITY			ACTIVITY:ACT_CITBETA_IDLEWALL"
		"		TASK_WAIT					30"
		"		TASK_SET_ACTIVITY			ACTIVITY:ACT_IDLE"
		""
		"	Interrupts"
	...
	(
		...
		"	Tasks"
		"		TASK_PLAY_SEQUENCE					ACTIVITY:ACT_CITBETA_MANHACKFLINCH"
		""
		"	Interrupts"
	)
