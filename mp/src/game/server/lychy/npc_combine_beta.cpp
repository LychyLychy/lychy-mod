...
static int g_fCombineQuestion;				// true if an idle grunt asked a question. Cleared when someone answers. YUCK old global from grunt code
...
const char *CNPC_CombineBeta::pCombineSentences[] = 
{
...
LINK_ENTITY_TO_CLASS( npc_combine_beta, CNPC_CombineBeta );

//---------------------------------------------------------
// Save/Restore
//---------------------------------------------------------
BEGIN_DATADESC( CNPC_CombineBeta )
...
CNPC_CombineBeta::CNPC_CombineBeta()
{
...
//------------------------------------------------------------------------------
void CNPC_CombineBeta::InputLookOff( inputdata_t &inputdata )
{
...
//------------------------------------------------------------------------------
void CNPC_CombineBeta::InputLookOn( inputdata_t &inputdata )
{
	m_spawnflags &= ~SF_COMBINE_NO_LOOK;
}
...
// Purpose:
//-----------------------------------------------------------------------------
void CNPC_CombineBeta::Precache()
{
...
extern ConVar lychy_combine_s_model;
...
//
//-----------------------------------------------------------------------------
void CNPC_CombineBeta::Spawn( void )
{
	SetHullType(HULL_HUMAN);
	SetHullSizeNormal();
	if (!strstr(GetClassname(), "_s"))
		SetModel(lychy_combine_s_model.GetString());
...
//-----------------------------------------------------------------------------
bool CNPC_CombineBeta::CreateBehaviors()
{
...
// Purpose: 
//-----------------------------------------------------------------------------
float CNPC_CombineBeta::MaxYawSpeed( void )
{
	switch( GetActivity() )
	{
	case ACT_TURN_LEFT:
...
// Output :
//-----------------------------------------------------------------------------
bool CNPC_CombineBeta::OverrideMoveFacing( const AILocalMoveGoal_t &move, float flInterval )
{
...
//
//-----------------------------------------------------------------------------
Class_T	CNPC_CombineBeta::Classify ( void )
{
...
// start task
//=========================================================
void CNPC_CombineBeta::StartTask( const Task_t *pTask )
{
	switch ( pTask->iTask )
	{
...
// RunTask
//=========================================================
void CNPC_CombineBeta::RunTask( const Task_t *pTask )
{
	/*
	{
...
// Output  :
//------------------------------------------------------------------------------
Vector CNPC_CombineBeta::BodyTarget( const Vector &posSrc, bool bNoisy ) 
{
...
// Purpose:
//------------------------------------------------------------------------------
bool CNPC_CombineBeta::FVisible( CBaseEntity *pEntity, int traceMask, CBaseEntity **ppBlocker )
{
...
//-----------------------------------------------------------------------------
bool CNPC_CombineBeta::UpdateEnemyMemory( CBaseEntity *pEnemy, const Vector &position, bool firstHand )
{
	if( m_spawnflags & SF_COMBINE_NO_LOOK )
	{
		return false;
...
//-----------------------------------------------------------------------------
void CNPC_CombineBeta::BuildScheduleTestBits( void )
{
	if (gpGlobals->curtime < m_flNextAttack)
	{
...
//-----------------------------------------------------------------------------
Activity CNPC_CombineBeta::NPC_TranslateActivity( Activity eNewActivity )
{
...
// Output :
//-----------------------------------------------------------------------------
int CNPC_CombineBeta::GetSoundInterests( void )
{
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_CombineBeta::CheckAmmo ( void )
{
	if (GetActiveWeapon())
	{
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_CombineBeta::AnnounceAssault(void)
{
...
// Output :
//-----------------------------------------------------------------------------
int CNPC_CombineBeta::SelectSchedule( void )
{
	...
	case NPC_STATE_COMBAT:
		{
			...
			if ( HasCondition(COND_NEW_ENEMY) )
			{
				...
				if ( m_pSquad && pEnemy )
				{
					...
					if ( m_pSquad->IsLeader( this ) || !pLeader || pLeader->GetEnemy() != pEnemy)
					{
...
int CNPC_CombineBeta::SelectScheduleAttack()
{
	// Kick attack?
...
// Output :
//-----------------------------------------------------------------------------
int CNPC_CombineBeta::TranslateSchedule( int scheduleType ) 
{
	switch( scheduleType )
	{
...
//=========================================================
//=========================================================
CBaseEntity *CNPC_CombineBeta::Kick( void )
{
	trace_t tr;
...
//=========================================================
void CNPC_CombineBeta::HandleAnimEvent( animevent_t *pEvent )
{
	Vector vecShootDir;
...
// Output :
//-----------------------------------------------------------------------------
Vector CNPC_CombineBeta::Weapon_ShootPosition( )
{
 	Vector right;
...
void CNPC_CombineBeta::SpeakSentence( int sentenceType )
{
	if		(sentenceType == 0)
	{
		if (FOkToMakeSound())
...
// PainSound
//=========================================================
void CNPC_CombineBeta::PainSound (const CTakeDamageInfo& info)
{
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_CombineBeta::LostEnemySound( void)
{
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_CombineBeta::FoundEnemySound( void)
{
	// High priority so don't check FOkToMakeSound()
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_CombineBeta::AlertSound( void)
{
...
// NotifyDeadFriend
//=========================================================
void CNPC_CombineBeta::NotifyDeadFriend ( CBaseEntity* pFriend )
{
...
// DeathSound 
//=========================================================
void CNPC_CombineBeta::DeathSound (const CTakeDamageInfo& info)
{
...
// IdleSound 
//=========================================================
void CNPC_CombineBeta::IdleSound( void )
{
...
// Output :
//-----------------------------------------------------------------------------
int	CNPC_CombineBeta::RangeAttack2Conditions( float flDot, float flDist ) 
{
...
// Output : int
//-----------------------------------------------------------------------------
int CNPC_CombineBeta::GetGrenadeConditions( float flDot, float flDist )
{
...
// Output :
//-----------------------------------------------------------------------------
int CNPC_CombineBeta::MeleeAttack1Conditions ( float flDot, float flDist )
{
	if (flDist > 64)
	{
...
// Output : Vector
//-----------------------------------------------------------------------------
Vector CNPC_CombineBeta::EyePosition( void ) 
{ 
	if ( m_bStanding )
	{
...
// Output : Vector
//-----------------------------------------------------------------------------
Vector CNPC_CombineBeta::EyeOffset( Activity nActivity )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
NPC_STATE CNPC_CombineBeta::SelectIdealState ( void )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_CombineBeta::OnBeginMoveAndShoot()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_CombineBeta::OnEndMoveAndShoot()
{
	VacateStrategySlot();
}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
WeaponProficiency_t CNPC_CombineBeta::CalcWeaponProficiency( CBaseCombatWeapon *pWeapon )
{
...
AI_BEGIN_CUSTOM_NPC( npc_combine_beta, CNPC_CombineBeta )
