...
//	>> CNPC_CombineBeta
//=========================================================
class CNPC_CombineBeta : public CAI_BaseHumanoid
{
	DECLARE_CLASS( CNPC_CombineBeta, CAI_BaseHumanoid );
...
private:
	...
	class CCombineStandoffBehavior : public CAI_ComponentWithOuter<CNPC_CombineBeta, CAI_StandoffBehavior>
	{
		typedef CAI_ComponentWithOuter<CNPC_CombineBeta, CAI_StandoffBehavior> BaseClass;
	...
	inline int SENTENCEG_PlayRndSz(edict_t* entity, const char* szrootname, float volume, soundlevel_t soundlevel, int flags, int pitch)
	{
		char* newrootname = (char*)(calloc(strlen(szrootname) + 20, sizeof(char)));
		strcpy(newrootname, "BETA_");
		strcat(newrootname, szrootname);
		return ::SENTENCEG_PlayRndSz(entity, newrootname, volume, soundlevel, flags, pitch);
	}
};
