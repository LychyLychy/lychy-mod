...
FACTORYCMD(lychy_combinegunship_beta, "npc_combinegunship", "npc_combinegunship_beta");
...
// Custom activities
//=====================================
static Activity ACT_GUNSHIP_PATROL;
static Activity ACT_GUNSHIP_HOVER;
static Activity ACT_GUNSHIP_CRASH;
...
BEGIN_DATADESC( CGunshipCrashFX )
	// Function Pointers
	DEFINE_FIELD( m_flStopExplodeTime, FIELD_TIME ),

	DEFINE_ENTITYFUNC(FXThink ),
END_DATADESC()
...
//===================================================================
class CNPC_CombineGunshipBeta : public CBaseHelicopter
{
public:
	DECLARE_CLASS( CNPC_CombineGunshipBeta, CBaseHelicopter );

	CNPC_CombineGunshipBeta( void );
...
LINK_ENTITY_TO_CLASS( npc_combinegunship_beta, CNPC_CombineGunshipBeta );

IMPLEMENT_SERVERCLASS_ST( CNPC_CombineGunshipBeta, DT_CombineGunshipBeta )
	SendPropVector(SENDINFO(m_vecHitPos), -1, SPROP_COORD),
END_SEND_TABLE()

BEGIN_DATADESC( CNPC_CombineGunshipBeta )

	DEFINE_ENTITYFUNC(  FlyTouch ),
	DEFINE_ENTITYFUNC(  ExplodeTouch ),

	DEFINE_FIELD(  m_flNextGroundAttack,FIELD_FLOAT ),
	DEFINE_FIELD(  m_bIsGroundAttacking,FIELD_BOOLEAN ),
	DEFINE_FIELD(  m_bCanGroundAttack,	FIELD_BOOLEAN ),
	DEFINE_FIELD(  m_flGroundAttackTime,FIELD_FLOAT ),
	DEFINE_FIELD(  m_flTargetPersistenceTime,FIELD_FLOAT ),

	DEFINE_FIELD(  m_pSmokeTrail,		FIELD_EHANDLE ),
	DEFINE_SOUNDPATCH(  m_pAirIntakeSound ),
	...
	DEFINE_FIELD(  m_bBurstHit,			FIELD_BOOLEAN ),
	DEFINE_FIELD(  m_nDebrisModel,		FIELD_INTEGER ),
	DEFINE_FIELD(  m_flTimeNextPing,	FIELD_FLOAT ),
	...
	DEFINE_INPUTFUNC(  FIELD_VOID, "DoGroundAttack", InputDoGroundAttack ),
...
// Constructor
//-----------------------------------------------------------------------------
CNPC_CombineGunshipBeta::CNPC_CombineGunshipBeta( void )
{ 
	m_pSmokeTrail	= NULL;
	m_iAmmoType		= -1; 
}
...
// Purpose:
//------------------------------------------------------------------------------
void CNPC_CombineGunshipBeta::Spawn( void )
{
	Precache( );

	SetModel( "models/gunship_beta.mdl" );
...
// Purpose:
//------------------------------------------------------------------------------
void CNPC_CombineGunshipBeta::Precache( void )
{
	engine->PrecacheModel("models/gunship_beta.mdl");
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_CombineGunshipBeta::Ping( void )
{
	if( IsCrashing() )
	{
		return;
...
// Output : float
//-----------------------------------------------------------------------------
float CNPC_CombineGunshipBeta::GroundDistToPosition( const Vector &pos )
{
	Vector vecDiff;
...
void CNPC_CombineGunshipBeta::PlayPatrolLoop( void )
{
...
void CNPC_CombineGunshipBeta::PlayAngryLoop( void )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_CombineGunshipBeta::HelicopterPostThink( void )
{
...
// Output : Vector
//-----------------------------------------------------------------------------
Vector CNPC_CombineGunshipBeta::GetGroundAttackHitPosition( void )
{
	trace_t	tr;
...
//-----------------------------------------------------------------------------
bool CNPC_CombineGunshipBeta::CheckGroundAttack( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_CombineGunshipBeta::StartGroundAttack( void )
{
	...
	//CEntityMessageFilter filter( this, "CNPC_CombineGunshipBeta" );
	//filter.MakeReliable();
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_CombineGunshipBeta::StopGroundAttack( void )
{
...
//-----------------------------------------------------------------------------
void CNPC_CombineGunshipBeta::DoCombat( void )
{
	float flDeltaSeen;
...
//-----------------------------------------------------------------------------
void CNPC_CombineGunshipBeta::MoveHead( void )
{
...
//			helicopters and such to scheduled AI. (sjb)
//-----------------------------------------------------------------------------
void CNPC_CombineGunshipBeta::PrescheduleThink( void )
{
...
// Output  : 
//------------------------------------------------------------------------------
bool CNPC_CombineGunshipBeta::FireGun( void )
{
	if ( m_lifeState != LIFE_ALIVE )
		return false;
...
//------------------------------------------------------------------------------
void CNPC_CombineGunshipBeta::FireCannonRound( void )
{
	Vector vecPenetrate;
	trace_t tr;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_CombineGunshipBeta::FVisible( CBaseEntity *pEntity, int traceMask, CBaseEntity **ppBlocker )
{
...
//-----------------------------------------------------------------------------
void CNPC_CombineGunshipBeta::InputSetPenetrationDepth( inputdata_t &inputdata )
{
...
//-----------------------------------------------------------------------------
void CNPC_CombineGunshipBeta::InputOmniscientOn( inputdata_t &inputdata )
{
	m_fOmniscient = true;
}
...
//-----------------------------------------------------------------------------
void CNPC_CombineGunshipBeta::InputOmniscientOff( inputdata_t &inputdata )
{
	m_fOmniscient = false;
}
...
//-----------------------------------------------------------------------------
void CNPC_CombineGunshipBeta::InputBlindfireOn( inputdata_t &inputdata )
{
	m_fBlindfire = true;
}
...
//-----------------------------------------------------------------------------
void CNPC_CombineGunshipBeta::InputBlindfireOff( inputdata_t &inputdata )
{
	m_fBlindfire = false;
}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_CombineGunshipBeta::BeginCrash( void )
{
	...
	SetTouch( &CNPC_CombineGunshipBeta::ExplodeTouch );
...
//-----------------------------------------------------------------------------
void CNPC_CombineGunshipBeta::Event_Killed( const CTakeDamageInfo &info )
{
...
//-----------------------------------------------------------------------------
void CNPC_CombineGunshipBeta::DyingThink( void )
{
	StudioFrameAdvance( );
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_CombineGunshipBeta::ApplyGeneralDrag( void )
{
...
void CNPC_CombineGunshipBeta::Flight( void )
{
...
//
//-----------------------------------------------------------------------------
void CNPC_CombineGunshipBeta::DoRotorWash( void )
{
	Vector vecForward;
	GetVectors( &vecForward, NULL, NULL );
...
// Output  :
//------------------------------------------------------------------------------
void CNPC_CombineGunshipBeta::InitializeRotorSound( void )
{
	CSoundEnvelopeController &controller = CSoundEnvelopeController::GetController();
...
// Output  :
//------------------------------------------------------------------------------
void CNPC_CombineGunshipBeta::UpdateRotorSoundPitch( int iPitch )
{
	CSoundEnvelopeController &controller = CSoundEnvelopeController::GetController();
...
// Output : 
//-----------------------------------------------------------------------------
void CNPC_CombineGunshipBeta::ApplySidewaysDrag( const Vector &vecRight )
{
...
//------------------------------------------------------------------------------
void CNPC_CombineGunshipBeta::SelfDestruct( void )
{
	SetThink( NULL );
...
// Output  :
//------------------------------------------------------------------------------
void CNPC_CombineGunshipBeta::InputSelfDestruct( inputdata_t &inputdata )
{
	SelfDestruct();
}
...
// Output  :
//------------------------------------------------------------------------------
void CNPC_CombineGunshipBeta::InputSetDockingBBox( inputdata_t &inputdata )
{
...
// Output  :
//------------------------------------------------------------------------------
void CNPC_CombineGunshipBeta::InputSetNormalBBox( inputdata_t &inputdata )
{
...
// Input  : &inputdata - 
//-----------------------------------------------------------------------------
void CNPC_CombineGunshipBeta::InputEnableGroundAttack( inputdata_t &inputdata )
{
...
// Input  : &inputdata - 
//-----------------------------------------------------------------------------
void CNPC_CombineGunshipBeta::InputDisableGroundAttack( inputdata_t &inputdata )
{
...
// Input  : &inputdata - 
//-----------------------------------------------------------------------------
void CNPC_CombineGunshipBeta::InputDoGroundAttack( inputdata_t &inputdata )
{
	StartGroundAttack();
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_CombineGunshipBeta::UpdateVehicularEnemyTarget( void )
{
...
// Input  : &vGunPosition - 
//-----------------------------------------------------------------------------
void CNPC_CombineGunshipBeta::UpdateEnemyTarget( void )
{
	Vector	vGunPosition;
...
// Output : Vector
//-----------------------------------------------------------------------------
Vector CNPC_CombineGunshipBeta::GetMissileTarget( void )
{
...
// Output  :
//------------------------------------------------------------------------------
Vector CNPC_CombineGunshipBeta::GetEnemyTarget( void )
{
...
// Input  : &tr - 
//-----------------------------------------------------------------------------
void CNPC_CombineGunshipBeta::DoCustomImpactEffect( const trace_t &tr )
{
...
//			iTracerType - 
//-----------------------------------------------------------------------------
void CNPC_CombineGunshipBeta::MakeTracer( const Vector &vecTracerSrc, const trace_t &tr, int iTracerType )
{
	switch ( iTracerType )
	{
	case TRACER_LINE:
...
// Output : int
//-----------------------------------------------------------------------------
void CNPC_CombineGunshipBeta::TraceAttack( const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr )
{
	// Reflect bullets
...
// Damage filtering
//------------------------------------------------------------------------------
int	CNPC_CombineGunshipBeta::OnTakeDamage_Alive( const CTakeDamageInfo &inputInfo )
{
	...
	if ( m_pSmokeTrail )
	{
		...
		m_pSmokeTrail->FollowEntity( this, GetModelPtr()->pAttachment(9-1 ).pszName());
	}
...
//------------------------------------------------------------------------------
void CNPC_CombineGunshipBeta::StartCannonBurst( int iBurstSize )
{
...
//------------------------------------------------------------------------------
void CNPC_CombineGunshipBeta::StopCannonBurst( void )
{
	m_bIsFiring = false;
	m_bPreFire = false;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_CombineGunshipBeta::StopLoopingSounds( void )
{
	CSoundEnvelopeController &controller = CSoundEnvelopeController::GetController();
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_CombineGunshipBeta::UpdateTrackNavigation( void )
{
...
//-----------------------------------------------------------------------------
void CNPC_CombineGunshipBeta::UpdateDesiredPosition( void )
{
...
// Input  : *pOther - 
//-----------------------------------------------------------------------------
void CNPC_CombineGunshipBeta::ExplodeTouch( CBaseEntity *pOther )
{
	SelfDestruct();
}
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_CombineGunshipBeta::IsValidEnemy( CBaseEntity *pEnemy )
{
...
//-----------------------------------------------------------------------------
bool CNPC_CombineGunshipBeta::IsTargettingMissile( void )
{
	if ( GetEnemy() == NULL )
		return false;
...
AI_BEGIN_CUSTOM_NPC( npc_combinegunship_beta, CNPC_CombineGunshipBeta )
