...
FACTORYCMD(lychy_manhack_beta, "npc_manhack", "npc_manhack_beta");
...
extern ConVar	sk_manhack_health;
...
BEGIN_DATADESC( CNPC_ManhackBeta )
	...
	DEFINE_FIELD(  m_flIgnoreDamageTime,		FIELD_TIME),
	...
	DEFINE_FIELD(  m_bWasClubbed,				FIELD_BOOLEAN),
...
LINK_ENTITY_TO_CLASS( npc_manhack_beta, CNPC_ManhackBeta );

IMPLEMENT_SERVERCLASS_ST(CNPC_ManhackBeta, DT_NPC_ManhackBeta)
END_SEND_TABLE()
...
// Output  :
//------------------------------------------------------------------------------
CNPC_ManhackBeta::CNPC_ManhackBeta()
{
#ifdef _DEBUG
...
//-----------------------------------------------------------------------------
Class_T	CNPC_ManhackBeta::Classify(void)
{
...
//-----------------------------------------------------------------------------
void CNPC_ManhackBeta::Event_Dying(void)
{
	SetHullSizeNormal();
...
// Purpose:
//-----------------------------------------------------------------------------
void CNPC_ManhackBeta::StopLoopingSounds(void)
{
	// Kill the engine!
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_ManhackBeta::PrescheduleThink( void )
{
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_ManhackBeta::TraceAttack( const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr )
{
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_ManhackBeta::Event_Killed( const CTakeDamageInfo &info )
{
	// turn off the blur!
	...
	// Sound
	CPASAttenuationFilter filter2( this, "NPC_ManhackBeta.Die" );
	EmitSound( filter2, entindex(), "NPC_ManhackBeta.Die" );
...
void CNPC_ManhackBeta::HitPhysicsObject( CBaseEntity *pOther )
{
...
void CNPC_ManhackBeta::VPhysicsShadowCollision( int index, gamevcollisionevent_t *pEvent )
{
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_ManhackBeta::CrashTouch( CBaseEntity *pOther )
{
...
// Output :
//-----------------------------------------------------------------------------
int	CNPC_ManhackBeta::OnTakeDamage_Alive( const CTakeDamageInfo &info )
{
...
//------------------------------------------------------------------------------
void CNPC_ManhackBeta::Ragdoll(void)
{
...
// Purpose:
//------------------------------------------------------------------------------
bool CNPC_ManhackBeta::CorpseGib( const CTakeDamageInfo &info )
{
	KillSprites( 0.0 );
...
// Output :
//-----------------------------------------------------------------------------
int	CNPC_ManhackBeta::OnTakeDamage_Dying( const CTakeDamageInfo &info )
{
...
//-----------------------------------------------------------------------------
void CNPC_ManhackBeta::OnStateChange( NPC_STATE OldState, NPC_STATE NewState )
{
...
// Input  :
//-----------------------------------------------------------------------------
int CNPC_ManhackBeta::SelectSchedule ( void )
{
...
// Input  : Type - 
//-----------------------------------------------------------------------------
void CNPC_ManhackBeta::HandleAnimEvent( animevent_t *pEvent )
{
...
//-----------------------------------------------------------------------------
bool CNPC_ManhackBeta::IsFlyingActivity( Activity baseAct )
{
...
// Input  : Type - 
//-----------------------------------------------------------------------------
Activity CNPC_ManhackBeta::NPC_TranslateActivity( Activity baseAct )
{
...
// Input  : Type - 
//-----------------------------------------------------------------------------
int CNPC_ManhackBeta::TranslateSchedule( int scheduleType ) 
{
	switch ( scheduleType )
	{
...
// Input  : flInterval - 
//-----------------------------------------------------------------------------
bool CNPC_ManhackBeta::OverrideMove(float flInterval)
{
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_ManhackBeta::TurnHeadRandomly(float flInterval )
{
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_ManhackBeta::MoveToTarget(float flInterval, const Vector &MoveTarget)
{
	if (flInterval <= 0)
	{
		return;
...
// Output :
//-----------------------------------------------------------------------------
int CNPC_ManhackBeta::MoveCollisionMask(void)
{
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_ManhackBeta::Splash( const Vector &vecSplashPos )
{
	EmitSound( "NPC_ManhackBeta.Splash" );
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_ManhackBeta::Slice( CBaseEntity *pHitEntity, float flInterval, trace_t &tr )
{
	...
	else if( pHitEntity->m_takedamage != DAMAGE_NO )
	{
		...
		EmitSound( "NPC_ManhackBeta.Slice" );
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_ManhackBeta::Bump( CBaseEntity *pHitEntity, float flInterval, trace_t &tr )
{
	...
	if (flInterval > 0)
	{
		...
		{
			EmitSound( "NPC_ManhackBeta.Grind" );
		}
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_ManhackBeta::CheckCollisions(float flInterval)
{
	// ----------------------------------------
...
void CNPC_ManhackBeta::PlayFlySound(void)
{
	float flEnemyDist;
	...
	if (gpGlobals->curtime > m_flNextEngineSoundTime && flEnemyDist < 48)
	{
		...
		EmitSound( "NPC_ManhackBeta.EngineNoise" );
	}
}
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_ManhackBeta::MoveExecute_Alive(float flInterval)
{
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_ManhackBeta::SpinBlades(float flInterval)
{
	if (!m_bBladesActive)
	{
...
//-----------------------------------------------------------------------------
void CNPC_ManhackBeta::MoveExecute_Dead(float flInterval)
{
	if( GetWaterLevel() > 0 )
	{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_ManhackBeta::Precache(void)
{
	//
	// Model.
	//
	engine->PrecacheModel("models/manhack_beta.mdl");
	engine->PrecacheModel( MANHACK_GLOW_SPRITE );
	PropBreakablePrecacheAll( MAKE_STRING("models/manhack_beta.mdl") );

	PrecacheScriptSound( "NPC_ManhackBeta.Die");
	PrecacheScriptSound("NPC_ManhackBeta.Bat");
	PrecacheScriptSound("NPC_ManhackBeta.Splash");
	PrecacheScriptSound("NPC_ManhackBeta.Slice");
	PrecacheScriptSound("NPC_ManhackBeta.Grind");
	PrecacheScriptSound("NPC_ManhackBeta.EngineNoise");
	PrecacheScriptSound("NPC_ManhackBeta.Unpack");
	PrecacheScriptSound("NPC_ManhackBeta.EngineSound1");
	PrecacheScriptSound("NPC_ManhackBeta.EngineSound2");
	PrecacheScriptSound("NPC_ManhackBeta.BladeSound");
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_ManhackBeta::GatherEnemyConditions( CBaseEntity *pEnemy )
{
...
// Output :
//-----------------------------------------------------------------------------
int CNPC_ManhackBeta::MeleeAttack1Conditions( float flDot, float flDist )
{
	if (flDist > 45)
	{
...
// Input  : *pTask - 
//-----------------------------------------------------------------------------
void CNPC_ManhackBeta::RunTask( const Task_t *pTask )
{
	switch ( pTask->iTask )
	{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_ManhackBeta::Spawn(void)
{
	Precache();

	SetModel( "models/manhack_beta.mdl" );
...
//-----------------------------------------------------------------------------
void CNPC_ManhackBeta::PostNPCInit( void )
{
...
void CNPC_ManhackBeta::BladesInit()
{
	if( !m_bBladesActive )
	{
...
// Crank up the engine!
//-----------------------------------------------------------------------------
void CNPC_ManhackBeta::StartEngine( bool fStartSound )
{
	if( fStartSound )
	{
		SoundInit();
...
//-----------------------------------------------------------------------------
void CNPC_ManhackBeta::SoundInit( void )
{
	...
	if( !m_pEngineSound1 )
	{
		m_pEngineSound1 = CSoundEnvelopeController::GetController().SoundCreate( filter, entindex(), "NPC_ManhackBeta.EngineSound1" );
	...
	if( !m_pEngineSound2 )
	{
		m_pEngineSound2 = CSoundEnvelopeController::GetController().SoundCreate( filter, entindex(), "NPC_ManhackBeta.EngineSound2" );
	...
	if( !m_pBladeSound )
	{
		m_pBladeSound = CSoundEnvelopeController::GetController().SoundCreate( filter, entindex(), "NPC_ManhackBeta.BladeSound" );
...
// Input  : pTask - 
//-----------------------------------------------------------------------------
void CNPC_ManhackBeta::StartTask( const Task_t *pTask )
{
	switch (pTask->iTask)
	{	
	case TASK_MANHACK_UNPACK:
		{
			...
			EmitSound( "NPC_ManhackBeta.Unpack" );
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_ManhackBeta::UpdateOnRemove( void )
{
	KillSprites( 1.0 );
...
//-----------------------------------------------------------------------------
bool CNPC_ManhackBeta::HandleInteraction(int interactionType, void* data, CBaseCombatCharacter* sourceEnt)
{
...
// Output : float
//-----------------------------------------------------------------------------
float CNPC_ManhackBeta::ManhackMaxSpeed( void )
{
	if( m_flWaterSuspendTime > gpGlobals->curtime )
	{
		// Slower in water!
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_ManhackBeta::ClampMotorForces( Vector &linear, AngularImpulse &angular )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_ManhackBeta::KillSprites( float flDelay )
{
	if( m_pEyeGlow )
	{
...
//			&tolerance - 
//-----------------------------------------------------------------------------
void CNPC_ManhackBeta::TranslateEnemyChasePosition( CBaseEntity *pEnemy, Vector &chasePosition, float &tolerance )
{
	Vector offset;
...
//
//-----------------------------------------------------------------------------
AI_BEGIN_CUSTOM_NPC( npc_manhack_beta, CNPC_ManhackBeta )
