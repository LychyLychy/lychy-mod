...
ConVar lychy_stalker_shouldbreathe("lychy_stalker_shouldbreathe", "0");
//Animation events
#define STALKER_AE_MELEE_HIT			1

extern ConVar sk_stalker_health;
extern ConVar sk_stalker_melee_dmg;
FACTORYCMD(lychy_stalker_beta, "npc_stalker", "npc_stalker_beta")

//ConVar	sk_stalker2003_health( "sk_stalker2003_health","0");
//ConVar	sk_stalker2003_melee_dmg( "sk_stalker2003_melee_dmg","0");

extern void		SpawnBlood(Vector vecSpot, int bloodColor, float flDamage);

LINK_ENTITY_TO_CLASS( npc_stalker_beta, CNPC_Stalker2003 );
...
BEGIN_DATADESC( CNPC_Stalker2003 )
...
// Output  :
//------------------------------------------------------------------------------
void CNPC_Stalker2003::SetScriptedTarget( CScriptedTarget *pScriptedTarget )
{
	if (pScriptedTarget)
	{
		// ---------------------------------------
...
// Output :
//-----------------------------------------------------------------------------
int	CNPC_Stalker2003::OnTakeDamage_Alive( const CTakeDamageInfo &inputInfo )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
float CNPC_Stalker2003::MaxYawSpeed( void )
{
	switch( GetActivity() )
	{
	case ACT_TURN_LEFT:
...
// Output : int
//-----------------------------------------------------------------------------
Class_T CNPC_Stalker2003::Classify( void )
{
...
//
//-----------------------------------------------------------------------------
void CNPC_Stalker2003::Spawn( void )
{
	Precache( );

	m_szAltClassName = AllocPooledString("npc_stalker");
	SetClassname("npc_stalker_beta");
	SetModel( "models/stalker_2003.mdl" );
...
//
//-----------------------------------------------------------------------------
void CNPC_Stalker2003::Precache( void )
{
	engine->PrecacheModel("models/stalker_2003.mdl");
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_Stalker2003::IdleSound ( void )
{
	if (gpGlobals->curtime > m_flNextBreatheSoundTime)
	{
		if(lychy_stalker_shouldbreathe.GetBool())
			SENTENCEG_PlayRndSz( edict(), "STALKER_BREATHING", STALKER_SENTENCE_VOLUME, SNDLVL_NORM, 0, 100);
		m_flNextBreatheSoundTime	= gpGlobals->curtime + 2;
	}
}

void CNPC_Stalker2003::OnScheduleChange()
{
	KillAttackBeam();
...
//			bitsDamageType - 
//-----------------------------------------------------------------------------
void CNPC_Stalker2003::Event_Killed( const CTakeDamageInfo &info )
{
	KillAttackBeam();
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_Stalker2003::DeathSound(const CTakeDamageInfo& info)
{ 
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_Stalker2003::PainSound(const CTakeDamageInfo& info)
{ 
	// Always play this sound
...
int CNPC_Stalker2003::GetSlotSchedule(int slotID)
{
	switch (slotID)
	{
...
void CNPC_Stalker2003::UpdateAttackBeam( void )
{
...
// start task
//=========================================================
void CNPC_Stalker2003::StartTask( const Task_t *pTask )
{
	switch ( pTask->iTask )
	{
...
// RunTask
//=========================================================
void CNPC_Stalker2003::RunTask( const Task_t *pTask )
{
	switch ( pTask->iTask )
	{
	case TASK_ANNOUNCE_ATTACK:
...
// Output :
//-----------------------------------------------------------------------------
int CNPC_Stalker2003::SelectSchedule( void )
{
	switch	( m_NPCState )
	{
		case NPC_STATE_IDLE:
		...
		case NPC_STATE_COMBAT:
		{
				 ...
				 HasCondition(COND_HEAVY_DAMAGE) ))
			{
				...
				if (HasCondition(COND_CAN_RANGE_ATTACK1))
				{
					...
					{
						SENTENCEG_PlayRndSz( edict(), "STALKER_ATTACK", STALKER_SENTENCE_VOLUME, SNDLVL_NORM, 0, 100);
...
// Output :
//-----------------------------------------------------------------------------
int CNPC_Stalker2003::TranslateSchedule( int scheduleType ) 
{
	switch( scheduleType )
	{
...
// Output  :
//------------------------------------------------------------------------------
Vector CNPC_Stalker2003::ScriptedBurnPosition(void)
{
...
// Output  :
//------------------------------------------------------------------------------
Vector CNPC_Stalker2003::LaserStartPosition(Vector vStalkerPos)
{
...
// Output  :
//------------------------------------------------------------------------------
void CNPC_Stalker2003::CalcBeamPosition(void)
{
...
void CNPC_Stalker2003::StartAttackBeam( void )
{
...
//		Msg( "Stalker Beam restart!\n" );
	...
	if (!m_pBeam)
	{
		...
		{
			EmitAmbientSound( GetWorldEntity()->entindex(), pEnemy->GetAbsOrigin(), "NPC_Stalker.AmbientLaserStart");
		}
		else
		{
			EmitAmbientSound( GetWorldEntity()->entindex(), GetAbsOrigin(), "NPC_Stalker.AmbientLaserStart");
		}
	}

	SetThink(&CNPC_Stalker2003::StalkerThink);
...
// Output  :
//------------------------------------------------------------------------------
void CNPC_Stalker2003::StalkerThink(void)
{
	DrawAttackBeam();
...
void CNPC_Stalker2003::DoSmokeEffect( const Vector &position )
{
...
// Output  :
//------------------------------------------------------------------------------
void CNPC_Stalker2003::DrawAttackBeam(void)
{
	if (!m_pBeam)
		return;
...
// Output  :
//------------------------------------------------------------------------------
void CNPC_Stalker2003::KillAttackBeam(void)
{
	if ( !m_pBeam )
		return;
...
// Output :
//-----------------------------------------------------------------------------
bool CNPC_Stalker2003::InnateWeaponLOSCondition( const Vector &ownerPos, const Vector &targetPos, bool bSetConditions )
{
	// --------------------
	// Check for occlusion
	// --------------------
...
// Output :
//-----------------------------------------------------------------------------
int CNPC_Stalker2003::MeleeAttack1Conditions ( float flDot, float flDist )
{
...
// Output :
//-----------------------------------------------------------------------------
int CNPC_Stalker2003::RangeAttack1Conditions( float flDot, float flDist )
{
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_Stalker2003::HandleAnimEvent( animevent_t *pEvent )
{
	switch( pEvent->event )
	{
...
//-----------------------------------------------------------------------------
bool CNPC_Stalker2003::FValidateHintType(CAI_Hint *pHint)
{
...
// Output :
//-----------------------------------------------------------------------------
Activity CNPC_Stalker2003::GetHintActivity( short sHintType )
{
...
// Output :
//-----------------------------------------------------------------------------
float	CNPC_Stalker2003::GetHintDelay( short sHintType )
{
	if (sHintType == HINT_WORLD_WORK_POSITION)
	{
		return 2.0;
...
void CNPC_Stalker2003::AddZigZagToPath(void) 
{
...
//-----------------------------------------------------------------------------
bool CNPC_Stalker2003::HandleInteraction(int interactionType, void *data, CBaseCombatCharacter* sourceEnt)
{
...
// Output  :
//------------------------------------------------------------------------------
CNPC_Stalker2003::CNPC_Stalker2003(void)
{
#ifdef _DEBUG
	m_vLaserDir.Init();
...
//------------------------------------------------------------------------------
/*
AI_BEGIN_CUSTOM_NPC(npc_stalker2003, CNPC_Stalker2003)
...
AI_SchedLoadStatus_t		CNPC_Stalker2003::gm_SchedLoadStatus = { true, -1 }; \
CAI_ClassScheduleIdSpace 	CNPC_Stalker2003::gm_ClassScheduleIdSpace; \
const char* CNPC_Stalker2003::gm_pszErrorClassName = "CNPC_Stalker2003"; \
\
CNPC_Stalker2003::CScheduleLoader::CScheduleLoader()\
{ \
CNPC_Stalker2003::LoadSchedules(); \
} \
\
/* --------------------------------------------- */ \
/* Load schedules for this type of NPC           */ \
/* --------------------------------------------- */ \
bool CNPC_Stalker2003::LoadSchedules(void)\
{\
return AI_DoLoadSchedules(CNPC_Stalker2003::BaseClass::LoadSchedules, \
	CNPC_Stalker2003::InitCustomSchedules, \
	& CNPC_Stalker2003::gm_SchedLoadStatus); \
}\
\
bool CNPC_Stalker2003::LoadedSchedules(void) \
{ \
return CNPC_Stalker2003::gm_SchedLoadStatus.fValid; \
}

CAI_LocalIdSpace 	CNPC_Stalker2003::gm_SquadSlotIdSpace; \
\
/* -------------------------------------------------- */ \
/* Given squadSlot enumeration return squadSlot name */ \
/* -------------------------------------------------- */ \
const char* CNPC_Stalker2003::SquadSlotName(int slotEN)\
{\
return gm_SquadSlotNamespace.IdToSymbol(CNPC_Stalker2003::gm_SquadSlotIdSpace.LocalToGlobal(slotEN)); \
}

void CNPC_Stalker2003::InitCustomSchedules(void) \
{ \
typedef CNPC_Stalker2003 CNpc; \
const char* pszClassName = "CNPC_Stalker2003"; \
\
...
taskIds.PushBack("TASK_STALKER_ZIGZAG", TASK_STALKER_ZIGZAG);
ActivityList_RegisterPrivateActivity("ACT_STALKER_WORK");
CAI_BaseNPC::AddActivityToSR("ACT_STALKER_WORK", ACT_STALKER_WORK);
squadSlotIds.PushBack("SQUAD_SLOT_CHASE_ENEMY_1", SQUAD_SLOT_CHASE_ENEMY_1);
squadSlotIds.PushBack("SQUAD_SLOT_CHASE_ENEMY_2", SQUAD_SLOT_CHASE_ENEMY_2);
scheduleIds.PushBack("SCHED_STALKER_RANGE_ATTACK", SCHED_STALKER_RANGE_ATTACK); \
const char* g_pszSCHED_STALKER_RANGE_ATTACK = \
"\n	Schedule" \
"\n"		 "SCHED_STALKER_RANGE_ATTACK" \
"	Tasks"
"		TASK_STOP_MOVING				0"
...
"		COND_ENEMY_DEAD" \
"\n"; \
schedulesToLoad.AddToTail(g_pszSCHED_STALKER_RANGE_ATTACK);

scheduleIds.PushBack("SCHED_STALKER_CHASE_ENEMY", SCHED_STALKER_CHASE_ENEMY); \
const char* g_pszSCHED_STALKER_CHASE_ENEMY = \
"\n	Schedule" \
"\n		" "SCHED_STALKER_CHASE_ENEMY" \
"	Tasks"
"		TASK_SET_FAIL_SCHEDULE			SCHEDULE:SCHED_CHASE_ENEMY_FAILED"
...
"		COND_HEAR_DANGER" \
"\n"; \
schedulesToLoad.AddToTail(g_pszSCHED_STALKER_CHASE_ENEMY);
