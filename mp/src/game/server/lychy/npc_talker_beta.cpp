...
BEGIN_DATADESC( CNPCSimpleTalkerBeta )
	DEFINE_FIELD(  m_useTime, FIELD_TIME ),
	DEFINE_USEFUNC(FollowerUse),
	// 							m_FollowBehavior (auto saved by AI)
END_DATADESC()

BEGIN_DATADESC( CAI_PlayerAllyBeta )

	// @TODO (toml 09-09-02): DEFINE_FIELD( CAI_PlayerAllyBeta, m_bitsSaid, FIELD_INTEGER ),
	DEFINE_FIELD(  m_hTalkTarget, FIELD_EHANDLE ),
	DEFINE_FIELD(  m_nSpeak, FIELD_INTEGER ),
	DEFINE_FIELD(  m_flLastHealthKitSearch, FIELD_FLOAT ),

	// Recalc'ed in Precache()
	//	DEFINE_FIELD( CAI_PlayerAllyBeta, m_voicePitch, FIELD_INTEGER ),
	//	DEFINE_FIELD( CAI_PlayerAllyBeta, m_szGrp, FIELD_??? ),
	DEFINE_FIELD( m_iszUse, FIELD_STRING ),
	DEFINE_FIELD( m_iszUnUse, FIELD_STRING ),
	//DEFINE_FIELD( CAI_PlayerAllyBeta, m_flLastSaidSmelled, FIELD_TIME ),
	// @TODO (toml 09-09-02): DEFINE_FIELD( CAI_PlayerAllyBeta, m_flStopTalkTime, FIELD_TIME ),
	// @TODO (toml 09-09-02): DEFINE_FIELD( CAI_PlayerAllyBeta, m_hTalkTarget, FIELD_EHANDLE ),
	// @TODO (toml 09-09-02): DEFINE_FIELD( CAI_PlayerAllyBeta, m_hMonologTalkTarget, FIELD_EHANDLE ),
	// @TODO (toml 09-09-02): DEFINE_FIELD( CAI_PlayerAllyBeta, m_iMonologIndex, FIELD_INTEGER ),
	// @TODO (toml 09-09-02): DEFINE_FIELD( CAI_PlayerAllyBeta, m_fMonologSuspended, FIELD_BOOLEAN ),
...
//-----------------------------------------------------------------------------
bool CAI_PlayerAllyBeta::IsPlayerAlly() 
{
#if 0
...
//-----------------------------------------------------------------------------
void CAI_PlayerAllyBeta::PlayerSelect( bool select )
{
...
char *CAI_PlayerAllyBeta::m_szFriends[TLK_CFRIENDS] = 
{
	"NPC_barney",
	"NPC_scientist",
...
Activity CAI_PlayerAllyBeta::NPC_TranslateActivity( Activity eNewActivity )
{
...
void CAI_PlayerAllyBeta::StartTask( const Task_t *pTask )
{
	switch ( pTask->iTask )
	{
	...
	case TASK_FIND_LOCK_HINTNODE_HEALTH:
		{
			...
			if (!GetHintNode())
			{
				SetHintNode(CAI_HintManager::FindHint(this, HINT_HEALTH_KIT, pTask->flTaskData, 1024));
			}
			if (GetHintNode() && GetHintNode()->Lock(this))
			{
				TaskComplete();
			}
			else
...
void CAI_PlayerAllyBeta::RunTask( const Task_t *pTask )
{
	switch( pTask->iTask )
	{
	case TASK_TALKER_CLIENT_STARE:
...
void CNPCSimpleTalkerBeta::Event_Killed( const CTakeDamageInfo &info )
{
...
void CAI_PlayerAllyBeta::Event_Killed( const CTakeDamageInfo &info )
{
...
CBaseEntity	*CAI_PlayerAllyBeta::EnumFriends( CBaseEntity *pPrevious, int listNumber, bool bTrace )
{
	...
	while (pFriend = gEntList.FindEntityByClassname( pFriend, pszFriend ))
	{
		...
		if ( bTrace )
		{
			...
			vecCheck.z = pFriend->CollisionProp()->CollisionSpaceMins().z;
...
void CAI_PlayerAllyBeta::AlertFriends( CBaseEntity *pKiller )
{
		...
		while (pFriend = EnumFriends( pFriend, i, true ))
		{
			...
			if ( pNPC->IsAlive() )
			{
				...
				else
				{
					...
					{
						// Killed by an enemy!!!
						CAI_PlayerAllyBeta *pAlly = (CAI_PlayerAllyBeta *)pNPC;
...
// Output :
//-----------------------------------------------------------------------------
CBaseEntity *CAI_PlayerAllyBeta::EyeLookTarget( void )
{
	if (GetExpresser()->IsSpeaking() && GetSpeechTarget() != NULL)
	{
...
void CAI_PlayerAllyBeta::ShutUpFriends( void )
{
	CBaseEntity *pFriend = NULL;
	int i;
...
void CNPCSimpleTalkerBeta::LimitFollowers( CBaseEntity *pPlayer, int maxFollowers )
{
	CBaseEntity *pFriend = NULL;
	int i, count;
	...
	for ( i = 0; i < TLK_CFRIENDS; i++ )
	{
		...
		{
			CAI_BaseNPC *pNPC = pFriend->MyNPCPointer();
			CNPCSimpleTalkerBeta *pTalker;
			if ( pNPC )
			{
				if ( pNPC->GetTarget() == pPlayer )
				{
					count++;
					if ( count > maxFollowers && (pTalker = dynamic_cast<CNPCSimpleTalkerBeta *>( pNPC ) ) != NULL )
...
/*
float CAI_PlayerAllyBeta::TargetDistance( void )
{
...
//=========================================================
void CAI_PlayerAllyBeta::HandleAnimEvent( animevent_t *pEvent )
{
	switch( pEvent->event )
	{		
...
// NPCs derived from CNPCSimpleTalkerBeta should call this in precache()

void CAI_PlayerAllyBeta::TalkInit( void )
{
...
// nearest player
//=========================================================
CBaseEntity *CAI_PlayerAllyBeta::FindNearestFriend(bool fPlayer)
{
	CBaseEntity *pFriend = NULL;
	...
	vecStart.z = CollisionProp()->CollisionSpaceMins().z;
	...
	for (i = cfriends-1; i > -1; i--)
	{
		...
		while (pFriend = gEntList.FindEntityByClassname( pFriend, pszFriend ))
		{
			...
			vecCheck.z = pFriend->CollisionProp()->CollisionSpaceMaxs().z;
...
void CAI_PlayerAllyBeta::Touch( CBaseEntity *pOther )
{
...
//=========================================================
void CAI_PlayerAllyBeta::IdleRespond( void )
{
...
// Output  :
//------------------------------------------------------------------------------
bool CAI_PlayerAllyBeta::IsOkToSpeak( void )
{
...
// Output  :
//------------------------------------------------------------------------------
bool CAI_PlayerAllyBeta::IsOkToCombatSpeak( void )
{
...
bool CAI_PlayerAllyBeta::CanPlaySentence( bool fDisregardState ) 
{ 
	if ( fDisregardState )
...
// FIdleStare
//=========================================================
int CAI_PlayerAllyBeta::FIdleStare( void )
{
	if (!IsOkToSpeak())
		return false;
...
//=========================================================
int CAI_PlayerAllyBeta::FIdleHello( void )
{
	if (!IsOkToSpeak())
		return false;
...
//=========================================================
int CAI_PlayerAllyBeta::FIdleSpeak ( void )
{ 
	...
	if (pFriend && !(pFriend->IsMoving()) && (random->RandomInt(0,99) < 60))
	{
		...
		CAI_PlayerAllyBeta *pTalkNPC = dynamic_cast<CNPCSimpleTalkerBeta *>(pFriend);
		if (pTalkNPC)
		{
...
int CNPCSimpleTalkerBeta::PlayScriptedSentence( const char *pszSentence, float delay, float volume, soundlevel_t soundlevel, bool bConcurrent, CBaseEntity *pListener )
{
...
int CAI_PlayerAllyBeta::PlayScriptedSentence( const char *pszSentence, float delay, float volume, soundlevel_t soundlevel, bool bConcurrent, CBaseEntity *pListener )
{
	if ( !bConcurrent )
		ShutUpFriends();
...
void CAI_PlayerAllyBeta::OnStartSpeaking()
{
...
void CAI_PlayerAllyBeta::SetAnswerQuestion( CAI_PlayerAllyBeta *pSpeaker )
{
	if ( !m_hCine )
...
int CAI_PlayerAllyBeta::OnTakeDamage_Alive( const CTakeDamageInfo &info )
{
	...
	if (info.GetAttacker() && (info.GetAttacker()->GetFlags() & FL_CLIENT) && info.GetDamage() < GetHealth() )
	{
		...
		if (pFriend && pFriend->IsAlive())
		{
			...
			CNPCSimpleTalkerBeta *pTalkNPC = (CNPCSimpleTalkerBeta *)pFriend;
...
int CAI_PlayerAllyBeta::TranslateSchedule( int scheduleType )
{
	switch( scheduleType )
	{
...
//=========================================================
void CAI_PlayerAllyBeta::GatherConditions( void )
{
...
void CAI_PlayerAllyBeta::PrescheduleThink( void )
{
...
void CAI_PlayerAllyBeta::TrySmellTalk( void )
{
	if ( !IsOkToSpeak() )
		return;
...
//---------------------------------------------------------
void CAI_PlayerAllyBeta::OnPlayerSelect()
{
	if ( IsOkToCombatSpeak() )
		Speak( TLK_SELECTED );
...
//---------------------------------------------------------
void CAI_PlayerAllyBeta::OnMoveOrder()
{
	if ( IsOkToCombatSpeak() )
		Speak( TLK_COMMANDED );
...
//---------------------------------------------------------
void CAI_PlayerAllyBeta::OnTargetOrder()
{
	if ( IsOkToCombatSpeak() )
		Speak( TLK_COMMANDED );
...
Disposition_t CAI_PlayerAllyBeta::IRelationType( CBaseEntity *pTarget )
{
#if 0 
	if ( pTarget->IsPlayer() )
		if ( m_afMemory & bits_MEMORY_PROVOKED )
...
void CAI_PlayerAllyBeta::OnChangeRunningBehavior( CAI_BehaviorBase *pOldBehavior,  CAI_BehaviorBase *pNewBehavior )
{
...
bool CAI_PlayerAllyBeta::OnBehaviorChangeStatus( CAI_BehaviorBase *pBehavior, bool fCanFinishSchedule )
{
...
void CNPCSimpleTalkerBeta::FollowerUse( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
...
bool CAI_PlayerAllyBeta::KeyValue( const char *szKeyName, const char *szValue )
{
...
void CAI_PlayerAllyBeta::Precache( void )
{
	/*
...
bool CAI_PlayerAllyBeta::ShouldSuspendMonolog( void )
{
	float flDist;
...
bool CAI_PlayerAllyBeta::ShouldResumeMonolog( void )
{
	float flDist;

	if( HasCondition( COND_SEE_PLAYER ) )
	{
		if (!GetExpresser()->GetMonologueTarget())
			return false;
		flDist = (GetExpresser()->GetMonologueTarget()->GetLocalOrigin() - GetLocalOrigin()).Length();
...
void CAI_PlayerAllyBeta::ModifyOrAppendCriteria( AI_CriteriaSet& set )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CAI_PlayerAllyBeta::OnKilledNPC( CBaseCombatCharacter *pKilled )
{
...
AI_BEGIN_CUSTOM_NPC(talk_monster_base,CAI_PlayerAllyBeta)
...
AI_BEGIN_CUSTOM_NPC(talk_monster,CNPCSimpleTalkerBeta)
AI_END_CUSTOM_NPC()
