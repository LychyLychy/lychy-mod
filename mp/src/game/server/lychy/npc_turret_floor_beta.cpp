...
class CTurretTipControllerBeta;
...
#define	FLOOR_TURRET_MODEL			"models/combine_turrets_beta/floor_turret.mdl"
...
FACTORYCMD(lychy_turret_floor_beta,"npc_turret_floor","npc_turret_floor_beta")
//Activities
static int ACT_FLOOR_TURRET_OPEN;
static int ACT_FLOOR_TURRET_CLOSE;
static int ACT_FLOOR_TURRET_OPEN_IDLE;
static int ACT_FLOOR_TURRET_CLOSED_IDLE;
static int ACT_FLOOR_TURRET_FIRE;
...
class CNPC_FloorTurretBeta : public CAI_BaseNPC
{
	DECLARE_CLASS( CNPC_FloorTurretBeta, CAI_BaseNPC );
public:
	
	CNPC_FloorTurretBeta( void );
	~CNPC_FloorTurretBeta( void );
...
protected:
	...
	CSprite					*m_pEyeGlow;
	CTurretTipControllerBeta	*m_pMotionController;
...
class CTurretTipControllerBeta : public CPointEntity, public IMotionEvent
{
	DECLARE_CLASS( CTurretTipControllerBeta, CPointEntity );
	DECLARE_DATADESC();

public:

	~CTurretTipControllerBeta( void );
	void Spawn( void );
	void Activate( void );
	...
	static CTurretTipControllerBeta	*CreateTipController( CNPC_FloorTurretBeta *pOwner )
	{
		if ( pOwner == NULL )
			return NULL;

		CTurretTipControllerBeta *pController = (CTurretTipControllerBeta *) Create( "floorturret_tipcontroller_beta", pOwner->GetAbsOrigin(), pOwner->GetAbsAngles() );
...
private:
	...
	CNPC_FloorTurretBeta			*m_pParentTurret;
};

//Datatable
BEGIN_DATADESC( CNPC_FloorTurretBeta )
	...
	DEFINE_FIELD(  m_pMotionController,FIELD_CLASSPTR ),
...
LINK_ENTITY_TO_CLASS( npc_turret_floor_beta, CNPC_FloorTurretBeta );

//-----------------------------------------------------------------------------
// Constructor
//-----------------------------------------------------------------------------
CNPC_FloorTurretBeta::CNPC_FloorTurretBeta( void )
{
	m_bActive			= false;
	m_pEyeGlow			= NULL;
	m_iAmmoType			= -1;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_FloorTurretBeta::UpdateOnRemove( void )
{
...
// Purpose: Precache
//-----------------------------------------------------------------------------
void CNPC_FloorTurretBeta::Precache( void )
{
	engine->PrecacheModel( FLOOR_TURRET_MODEL );	
	engine->PrecacheModel( FLOOR_TURRET_GLOW_SPRITE );

	PrecacheScriptSound("NPC_FloorTurretBeta.Alarm");
	PrecacheScriptSound("NPC_FloorTurretBeta.Ping");
	PrecacheScriptSound("NPC_FloorTurretBeta.Deploy");
	PrecacheScriptSound("NPC_FloorTurretBeta.Move");
	PrecacheScriptSound("NPC_FloorTurretBeta.Activate");
	PrecacheScriptSound("NPC_FloorTurretBeta.Alert");
	PrecacheScriptSound("NPC_FloorTurretBeta.ShotSounds");
	PrecacheScriptSound("NPC_FloorTurretBeta.Die");
	PrecacheScriptSound("NPC_FloorTurretBeta.Retract");
	PrecacheScriptSound("NPC_FloorTurretBeta.Retire");
...
//-----------------------------------------------------------------------------
void CNPC_FloorTurretBeta::Spawn( void )
{ 
	Precache();
	...
	{
		SetThink(&CNPC_FloorTurretBeta::AutoSearchThink );
	...
	// Activities
	ADD_CUSTOM_ACTIVITY( CNPC_FloorTurretBeta, ACT_FLOOR_TURRET_OPEN );
	ADD_CUSTOM_ACTIVITY( CNPC_FloorTurretBeta, ACT_FLOOR_TURRET_CLOSE );
	ADD_CUSTOM_ACTIVITY( CNPC_FloorTurretBeta, ACT_FLOOR_TURRET_CLOSED_IDLE );
	ADD_CUSTOM_ACTIVITY( CNPC_FloorTurretBeta, ACT_FLOOR_TURRET_OPEN_IDLE );
	ADD_CUSTOM_ACTIVITY( CNPC_FloorTurretBeta, ACT_FLOOR_TURRET_FIRE );
	
	SetUse( &CNPC_FloorTurretBeta::ToggleUse );
...
bool CNPC_FloorTurretBeta::CreateVPhysics( void )
{
	...
	m_pMotionController = CTurretTipControllerBeta::CreateTipController( this );
...
//-----------------------------------------------------------------------------
void CNPC_FloorTurretBeta::Retire( void )
{
	...
	if ( GetActivity() != ACT_FLOOR_TURRET_CLOSE )
	{
		...
		if ( UpdateFacing() == false )
		{
			...
			EmitSound( "NPC_FloorTurretBeta.Retire" );
...
//-----------------------------------------------------------------------------
void CNPC_FloorTurretBeta::Deploy( void )
{
	...
	if ( GetActivity() != ACT_FLOOR_TURRET_OPEN )
	{
		...
		EmitSound( "NPC_FloorTurretBeta.Deploy" );
	...
	if ( IsActivityFinished() )
	{
		...
		m_flPlaybackRate = 0;
		SetThink(&CNPC_FloorTurretBeta::SearchThink );

		EmitSound( "NPC_FloorTurretBeta.Move" );
	}
...
//-----------------------------------------------------------------------------
float CNPC_FloorTurretBeta::MaxYawSpeed( void )
{
...
//-----------------------------------------------------------------------------
bool CNPC_FloorTurretBeta::UpdateFacing( void )
{
	bool  bMoved = false;
...
//-----------------------------------------------------------------------------
void CNPC_FloorTurretBeta::SuppressThink( void )
{
	...
	if ( pEnemy != NULL )
	{
		SetThink(&CNPC_FloorTurretBeta::ActiveThink );
		return;
	}
...
//-----------------------------------------------------------------------------
void CNPC_FloorTurretBeta::ActiveThink( void )
{
	//Allow descended classes a chance to do something before the think function
	...
	if ( ( bEnemyVisible == false ) || ( flDistToEnemy > FLOOR_TURRET_RANGE ))
	{
		...
		ClearEnemyMemory();
		SetEnemy( NULL );
		SetThink(&CNPC_FloorTurretBeta::SuppressThink );
...
//-----------------------------------------------------------------------------
void CNPC_FloorTurretBeta::SearchThink( void )
{
	//Allow descended classes a chance to do something before the think function
	...
	if ( GetEnemy() != NULL )
	{
		...
		SpinUp();
		EmitSound( "NPC_FloorTurretBeta.Activate" );
		return;
	}
 	...
 	if ( gpGlobals->curtime > m_flLastSight )
	{
		...
		m_flLastSight = 0;
		SetThink(&CNPC_FloorTurretBeta::Retire );
		return;
	}
...
//-----------------------------------------------------------------------------
void CNPC_FloorTurretBeta::AutoSearchThink( void )
{
	//Allow descended classes a chance to do something before the think function
	...
	if ( GetEnemy() != NULL )
	{
		SetThink(&CNPC_FloorTurretBeta::Deploy );
		EmitSound( "NPC_FloorTurretBeta.Alert" );
	}
}
...
// Purpose: Fire!
//-----------------------------------------------------------------------------
void CNPC_FloorTurretBeta::Shoot( const Vector &vecSrc, const Vector &vecDirToEnemy )
{
	if ( GetEnemy() != NULL )
	{
		...
		EmitSound( "NPC_FloorTurretBeta.ShotSounds" );
...
//-----------------------------------------------------------------------------
bool CNPC_FloorTurretBeta::IsValidEnemy( CBaseEntity *pEnemy )
{
...
//-----------------------------------------------------------------------------
void CNPC_FloorTurretBeta::TippedThink( void )
{
	SetNextThink( gpGlobals->curtime + 0.05f );
	...
	else
	{
		...
		if ( GetActivity() != ACT_FLOOR_TURRET_CLOSE )
		{
			...
			if ( UpdateFacing() == false )
			{
				...
				EmitSound( "NPC_FloorTurretBeta.Die" );
				SpinDown();

				SetActivity( (Activity) ACT_FLOOR_TURRET_CLOSE );
				EmitSound( "NPC_FloorTurretBeta.Retract" );
...
//-----------------------------------------------------------------------------
inline bool CNPC_FloorTurretBeta::OnSide( void )
{
	Vector	up;
...
//-----------------------------------------------------------------------------
bool CNPC_FloorTurretBeta::PreThink( turretState_e state )
{
	//Animate
	StudioFrameAdvance();
	...
	else
	{
		...
		SetThink(&CNPC_FloorTurretBeta::TippedThink );
		...
		SpinUp();
		EmitSound( "NPC_FloorTurretBeta.Alarm" );
...
//-----------------------------------------------------------------------------
void CNPC_FloorTurretBeta::SetEyeState( eyeState_t state )
{
...
//-----------------------------------------------------------------------------
void CNPC_FloorTurretBeta::Ping( void )
{
	...
	//Ping!
	EmitSound( "NPC_FloorTurretBeta.Ping" );
...
//-----------------------------------------------------------------------------
void CNPC_FloorTurretBeta::Toggle( void )
{
...
//-----------------------------------------------------------------------------
void CNPC_FloorTurretBeta::Enable( void )
{
	if ( IsAlive() == false )
		return;
...
//-----------------------------------------------------------------------------
void CNPC_FloorTurretBeta::Disable( void )
{
	if ( IsAlive() == false )
		return;
...
//-----------------------------------------------------------------------------
void CNPC_FloorTurretBeta::InputToggle( inputdata_t &inputdata )
{
	Toggle();
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_FloorTurretBeta::InputEnable( inputdata_t &inputdata )
{
	Enable();
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_FloorTurretBeta::InputDisable( inputdata_t &inputdata )
{
	Disable();
}
...
//-----------------------------------------------------------------------------
void CNPC_FloorTurretBeta::ToggleUse ( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	switch( useType )
	{
	case USE_OFF:
...
//-----------------------------------------------------------------------------
void CNPC_FloorTurretBeta::TraceAttack( const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr )
{
...
// Input  : &info - 
//-----------------------------------------------------------------------------
int CNPC_FloorTurretBeta::OnTakeDamage( const CTakeDamageInfo &info )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_FloorTurretBeta::SpinUp( void )
{
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_FloorTurretBeta::SpinDown( void )
{
}
...
LINK_ENTITY_TO_CLASS( floorturret_tipcontroller_beta, CTurretTipControllerBeta );


//---------------------------------------------------------
// Save/Restore
//---------------------------------------------------------
BEGIN_DATADESC( CTurretTipControllerBeta )
...
CTurretTipControllerBeta::~CTurretTipControllerBeta()
{
	if ( m_pController )
	{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTurretTipControllerBeta::Spawn( void )
{
	m_bEnabled = true;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTurretTipControllerBeta::Activate( void )
{
	BaseClass::Activate();
...
//-----------------------------------------------------------------------------
IMotionEvent::simresult_e CTurretTipControllerBeta::Simulate( IPhysicsMotionController *pController, IPhysicsObject *pObject, float deltaTime, Vector &linear, AngularImpulse &angular )
{
	if ( Enabled() == false )
	{
		linear.Init();
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTurretTipControllerBeta::Enable( bool state )
{
	m_bEnabled = state;
}
...
// Input  : time - 
//-----------------------------------------------------------------------------
void CTurretTipControllerBeta::Suspend( float time )
{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CTurretTipControllerBeta::Enabled( void )
{
